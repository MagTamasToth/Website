...
### 4. Eigene Domain einrichten (www.metallagentur.at)
...
GitHub Pages → Custom domain: `www.metallagentur.at` eintragen → **Enforce HTTPS** aktivieren.
...
```bash
find . -name "*.html" -o -name "*.xml" | xargs sed -i 's|www.metallagentur.at|NEUE-DOMAIN.at|g'
```
